...
async function getProduct(id: string) {
  const res = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3002"}/api/products/${encodeURIComponent(id)}`, { cache: "no-store" });
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <Link href={product.printify_url} target="_blank" rel="noopener noreferrer" className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-lg">Buy on Store</Link>
