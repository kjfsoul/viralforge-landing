...
export class ProductsService {
  ...
  constructor() {
    this.csvPath = path.join(process.cwd(), 'Popup_Store_Products__parsed_ (1).csv');
  }
  ...
  private categorizeBrand(csvProduct: CSVProduct): string {
    ...
    switch (shopTitle) {
      ...
      default:
        ...
        const blob = `${csvProduct.product_title} ${csvProduct.product_description_text} ${csvProduct.tags}`.toLowerCase();
  ...
  private transformCSVToProducts(csvProducts: CSVProduct[]): Product[] {
    return csvProducts.map((row) => {
      ...
      const isUrgent = row.product_title?.toLowerCase().includes('exclusive') || 
  ...
  private determineCategory(csvProduct: CSVProduct): string {
    ...
    const description = csvProduct.product_description_text?.toLowerCase() || '';
    
    if (title.includes('t-shirt') || title.includes('shirt') || title.includes('apparel')) {
      return 'Apparel';
    }
    if (title.includes('poster') || title.includes('print') || title.includes('art')) {
      return 'Art & Prints';
    }
  ...
  async syncWithPrintify(shopId: string, blueprintId: number, printProviderId: number, variantId: number): Promise<any[]> {
    try {
      const { getPrintifyProductService } = await import('./printify-product-service');
