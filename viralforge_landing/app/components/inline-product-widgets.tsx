...
export function InlineProductWidget({ productId, context = 'general' }: { productId?: string, context?: string }) {
...
export function FloatingProductRecommendation() {
  ...
  useEffect(() => {
    const fetchHotProduct = async () => {
      try {
        const response = await fetch('/api/products?context=hero&featured=true')
        const data = await response.json()
        if (data.success && data.data.length > 0) {
          const urgentProduct = data.data.find((p: Product) => p.urgent) || data.data[0]
        ...
        <CardContent className="p-4">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center space-x-1">
              ...
              <span className="text-xs font-bold text-white">Mars Flyby Exclusive!</span>
            </div>
            <button
