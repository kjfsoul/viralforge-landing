...
const AUTH = process.env.IMPORTER_BEARER || ""; // optional bearer
...
export async function POST(req: NextRequest) {
  ...
  const items = rows
    .filter(
      (r) =>
        (r.storefront_product_url || "").trim() && Number(r.variant_count) > 0
    )
    .map((r) => ({
  ...
  // write a single public blob your frontend can read
